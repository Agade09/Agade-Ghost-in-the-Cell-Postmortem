...
One of the first things I did was code an Arena program([https://github.com/Agade09/CG-GitC-Arena](github link)) that could play games between two of my AIs. The Arena runs the binaries of the two competing AIs, feeds them inputs and recieves the outputs just like Codingame must do it on their servers. After every game the Arena prints the win rate of the first AI against the second AI with an error bar. Error bars on win rate can be calculated using [http://www.sigmazone.com/binomial_confidence_interval.htm](Bernouilli confidence intervals). The same thing [http://cgstats.magusgeek.com/app](cgstats) does to give you an error bar on the win rate.
...
In this game the smallest advantage over your opponent could allow you to get an extra point of production, gain even more advantage and so forth exponentially until your enemy's position is hopeless. So alot of my thinking was in terms of investement,cost and time to return on investment.
